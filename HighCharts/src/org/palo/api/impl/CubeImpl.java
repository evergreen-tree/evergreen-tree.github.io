 ...
 * @author Arnd Houben
 * @author Stepan Rutz
 * @version $Id: CubeImpl.java,v 1.100 2008/04/15 10:28:58 ArndHouben Exp $
 */
...
//		return rule;
	...
	public final Rule addRule(String definition, String externalIdentifier, boolean useIt, String comment) {
		RuleInfo rule = ruleLoader.create(definition, externalIdentifier, useIt, comment);
		Rule newRule = createRule(rule);
		fireRulesAdded(new Rule[]{newRule});
		return newRule;
//
//		checkRuleSupport();
//		RuleInfo ruleInfo = dbConnection.createRule(cubeInfo, definition,externalIdentifier,useIt,comment);
//		Rule rule = createRule(ruleInfo);
//		return rule;
	}
...
//			return true;
//		}
////		ConnectionImpl cimpl = (ConnectionImpl) getDatabase().getConnection();
////		if (cimpl.getConnectionInternal().deleteRule(getDatabase().getName(),
////				getName(), rule.getId())) {
    ...
    public final Cell[] getCells(Element[][] coordinates) {
		...
		try {
			...
			for(int i=0;i<_cells.length;++i) {
				if(_cells[i].getCoordinate().length < 1)
