...
public class CubeInfoBuilder {
	...
	private CellInfo parseValueNode(Node node, String [] strCoord) {
		...
		if (doubleType) {
			return new CellInfoImpl(CellInfo.TYPE_NUMERIC, true, new Double(text), strCoord);
		} else {
			return new CellInfoImpl(CellInfo.TYPE_STRING, true, text, strCoord);
		}		
	}
	...
	public CellInfo getData(String conName, XMLAClient client, XMLACubeInfo cube, XMLAElementInfo [] coordinates, XMLAConnection connection) {		
		...
		return new CellInfoImpl(CellInfo.TYPE_NUMERIC, false, "", strCoord);
	}
	...
	public CellInfo [] getDataArray(String conName, XMLAClient client, XMLACubeInfo cube, XMLAElementInfo[][] elements, XMLAConnection connection) {
		...
		if (elements == null || elements.length == 0 || elements[0] == null) {
			return new CellInfo [] {new CellInfoImpl(CellInfo.TYPE_NUMERIC, false, "", null)};
		}
...
//			System.err.println(sb.toString() + "\n");
			...
			if (faults != null && faults.getLength() != 0) {
				...
				cInfo[0] = new CellInfoImpl(CellInfo.TYPE_ERROR, true, result, new String [] {});
				return cInfo;
			}
			...
			for (int i = 0; i < totalElements; i++) {
				resultValues[i] = new CellInfoImpl(CellInfo.TYPE_NUMERIC, false, "", null);
			}
...
//			System.out.println("GetDataBulk ==\n" + sb.toString());
			...
			for (int i = 0, n = elements.length; i < n; i++) {
				...
				if (resValues[i] == null) {
					resValues[i] = new CellInfoImpl(CellInfo.TYPE_NUMERIC, false, "", coords);
				} else {
