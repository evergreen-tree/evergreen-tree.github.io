 ...
 * 
 * @author ArndHouben
 * @version $Id: HttpConnection.java,v 1.39 2008/05/20 10:45:01 ArndHouben Exp $
 */
...
//	public final DimensionInfo [] getDimensions(HierarchyInfo hierarchy) {
//		// TODO  Far too slow!! It's a one-to-one mapping, so when getting
//		// all hierarchies, we can cache the dimensions, can't we?
//		DimensionInfo [] dims = getDimensions(hierarchy.getDatabase());
//		for (int i = 0; i < dims.length; i++) {
//			if (dims[i].getId().equals(hierarchy.getId())) {
//				return new DimensionInfo [] {dims[i]};
//			}
//		}
//		return new DimensionInfo[0];
//	}
//	
//	public final HierarchyInfo [] getHierarchies(DatabaseInfo database) {
//		DimensionInfo [] dims = getDimensions(database);		
//		HierarchyInfoImpl [] hiers = new HierarchyInfoImpl[dims.length];
//		for (int i = 0; i < dims.length; i++) {
//			hiers[i] = new HierarchyInfoImpl(database, dims[i].getName(), dims[i].getId(), dims[i].getType());
//		}
//		return hiers;
//	}
//
//	public final HierarchyInfo [] getHierarchies(CubeInfo cube) {
//		String [] dimIds = cube.getDimensions();
//		DimensionInfo [] dims = new DimensionInfo[dimIds.length];
//		HierarchyInfoImpl [] hiers = new HierarchyInfoImpl[dimIds.length];
//		for (int i = 0; i < dims.length; i++) {
//			dims[i] = getDimension(cube.getDatabase(), dimIds[i]);
//			hiers[i] = new HierarchyInfoImpl(cube, dims[i].getName(), dims[i].getId(), dims[i].getType());
//		}
//		return hiers;
//	}
//	
//	public HierarchyInfo getHierarchy(DatabaseInfo database, String id) {
//		DimensionInfo dim = getDimension(database, id);		
//		return new HierarchyInfoImpl(database, dim.getName(), id, dim.getType());
//	}
//	
//	public HierarchyInfo getHierarchy(CubeInfo cube, String id) {
//		DimensionInfo dim = getDimension(cube.getDatabase(), id);
//		return new HierarchyInfoImpl(cube, dim.getName(), id, dim.getType());
//	}
...
//	}
	...
	public void update(RuleInfo rule, String definition, 
		...
		try {
			setOwnChange(true);
			RuleHandler ruleHandler = handlerRegistry.getRuleHandler();
			ruleHandler.update(rule, definition, externalIdentifier,
					useIt, comment);
		} catch (IOException e) {
			throw new PaloException(e.getLocalizedMessage(), e);
		}
