...
/**
 * <code></code> TODO DOCUMENT ME
 * 
 * @author ArndHouben
 * @version $Id: CellHandler.java,v 1.11 2008/03/04 08:51:10 ArndHouben Exp $
 */
public class CellHandler extends HttpHandler {
	...
	public final CellInfo[] getCellArea(CubeInfo cube, ElementInfo[][] coordinates) throws IOException {
	...
	public final boolean copyCell(CubeInfo cube, ElementInfo[] fromCoordinate, ElementInfo[] targetCoordinate,
			Object targetValue) {
		throw new PaloException("Currently not supported!!");
	}
	...
	public final boolean replaceValues(CubeInfo cube, ElementInfo[][] coordinates, Object[] newValues, boolean add,
			int splashMode, boolean notifyProcessors) throws IOException {
		DatabaseInfo database = cube.getDatabase();
	 ...
	 * 
	 * @param cube
	 *            the cube which contains the cell
	 * @param coordinate
	 *            cell coordinate within the cube
	...
	public final CellInfo getValue(CubeInfo cube, ElementInfo[] coordinate) throws IOException {
		...
		if (response[0].length == 4)
			cell.setRule(response[0][3]);
		return cell;
	}

	// /cell/values Shows datatype and value of a list of cube cells. cube
	public final CellInfo[] getValues(CubeInfo cube, ElementInfo[][] area) throws IOException {
		DatabaseInfo database = cube.getDatabase();
		...
		for (int i = 0; i < response.length; ++i) {
			cells[i] = cellBuilder.create(null, response[i]);
			if (response[i].length == 4)
				cells[i].setRule(response[i][3]);
		}
		return cells;
	}

	public final CellInfo[] export(CubeInfo cube, ExportContextInfo context) throws IOException {
		DatabaseInfo database = cube.getDatabase();
		...
		if (response.length == 1 && response[0].length == 3)
			throw new PaloException("getDataExport failed: " + response[0][0] + ", " + response[0][1] + ", "
					+ response[0][2]);
		...
		context.setProgress((Double.parseDouble(response[lastCell][0])) / (Double.parseDouble(response[lastCell][1])));
		return cells;
	}
